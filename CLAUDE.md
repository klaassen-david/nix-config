...
- **Failures are announced.** Every backup unit merges `backupRepo.alertHook`: `OnFailure=backup-alert@%n.service` (journal excerpt + critical `notify-send` to dk) plus an `ExecStartPost` that clears the flag on success. The alert writes `/var/lib/backup-alerts/<unit>`, which the sway bar polls (`home-manager/modules/sway/i3status-rust.nix`) — that stamp is what survives a failure nobody was logged in for. There is no flag to turn this off. `restic backup`/`forget`/`check` all pass `--retry-lock`, so the two jobs colliding after a boot is a wait, not a failure.
